...
class CocoDataset(Dataset):
    ...
    def load_image(self, image_index):
        ...
        # 不解压直接获取图片
        zip_file_path = os.path.join(self.root_dir, self.set_name + ".zip")
        val_z = zipfile.ZipFile(zip_file_path)
        img = mx.image.imdecode(val_z.read('%s/%s' % (self.set_name, image_info['file_name']))).asnumpy()
...
class Resizer(object):
    ...
    def __call__(self, sample, min_side=512, max_side=600):
