...
class IoULoss(nn.Module):
    ...
    def forward(self, pred_box, true_box):
        ...
        iou = inter_area / (pred_area + true_area - inter_area + 1e-4)
        iou = torch.clamp(iou, min=1e-6)
        loss = -torch.log(iou)
        #loss = loss.mean()
        #print("iou:%f" % (loss.mean()))
        return loss
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        N = inputs.size(0)
        C = inputs.size(1)
        class_mask = inputs.data.new(N, C).fill_(1)
        indicies = targets.eq(1)
        class_mask = class_mask.masked_fill(indicies, -1)
        p = class_mask * inputs
        
        class_mask2 = inputs.data.new(N, C).fill_(0)
