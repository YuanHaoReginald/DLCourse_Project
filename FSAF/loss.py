...
class IoULoss(nn.Module):
    def forward(self, pred, target):
        pred_left = pred[:, 0].float()
        pred_top = pred[:, 1].float()
        pred_right = pred[:, 2].float()
        pred_bottom = pred[:, 3].float()

        target_left = target[:, 0].float()
        target_top = target[:, 1].float()
        target_right = target[:, 2].float()
        target_bottom = target[:, 3].float()
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        mask = -inputs + 1
        mask.cuda()
        indices = targets.eq(1)
        p = inputs.masked_scatter(indices, mask)
        if self.alpha:
            alpha_mask = inputs.data.new(N, C).fill_(1 - self.alpha)
